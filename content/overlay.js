		...
		while (currentElementChild)
		{
			if (currentElementChild.nodeType == 1) {
				...
				if (currentElementChild.getAttribute) {
					role = currentElementChild.getAttribute("role");
				}
				...
				if (role && ARIA_LANDMARKS.isLandmark(role, currentElementChild)) {
					...
					if (this.menu) {
						...
						if (this.isDescendant(lastLandmarkedElement, currentElementChild)) ++depth;
						for (var j=0; j < depth; ++j) role = "-" + role;

						var label = ARIA_LANDMARKS.getLabel(currentElementChild);
						if (label != null) role = role + ": " + label;
